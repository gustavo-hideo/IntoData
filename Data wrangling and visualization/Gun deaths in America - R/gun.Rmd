...
```{r, include=FALSE}
base <- RCurl::getURL("https://raw.githubusercontent.com/fivethirtyeight/guns-data/master/full_data.csv")
gun <- read.csv(text = base)
...
```{r, include=FALSE}
num <- c(1, 1, 1, 1)
cat <- c("Black", "White", "Homicide", "Suicide")
type <- c("race", "race", "intent", "intent")

legend <- data.frame(do.call("cbind", list(cat, num, type)))
colnames(legend) <- c("category", "num", "type")



legend.plot <- legend %>%
  ggplot(aes(area = as.numeric(num), fill = category, label = category)) +
  geom_treemap() +
  geom_treemap_text(colour = "white", place = "centre") +
  scale_fill_manual(values = c("tomato", "gray55", "gray35", "steelblue"), guide = F) +
  labs(title = "Legend")


print(legend.plot, vp = grid::viewport(width = 0.4, height = 0.4, just = c("right", "bottom")))
...
death %>%
 mutate(text = ifelse(intent == "Homicide", "           Homicides  x", "  66.7% of gun deaths are suicides")
       ,text2 = ifelse(intent == "Homicide", "Suicides           ", "")) %>%
 ggplot(aes(area = n, fill = intent, label = text)) +
 geom_treemap() +
 geom_treemap_text(colour = "white", place = "left", size = 52.4) +
 scale_fill_manual(values = c("gray35", "indianred3"), guide = F) +
 scale_color_manual(values = c("gray55", "gray35"), guide = F) +
 geom_treemap_text(aes(label = text2), colour = "indianred3", place = "right", size = 52.4)


death %>%
 ggplot(aes(area = n, fill = intent)) +
 geom_treemap() +
 scale_fill_manual(values = c("gray55", "gray35"), guide = F) +
 labs(title = "66.7% of gun deaths are suicides")
